...
# default SSL certificates extra options
...
printCSVJSON() {
  parseCSV "$1" | while IFS=';' read -r LINE; do
    # unset results set json
    unset JSON

    # split in fiels the line
    ...
    # compose the json
    [ ! -z "${HOST}" ]     && JSON+=" \"{#URL}\":\"${SCHEMA}://${HOST}${URL_PORT}${RPATH}\","
    [ ! -z "${SCHEMA}" ]   && JSON+=" \"{#SCHEMA}\":\"${SCHEMA}\","
    [ ! -z "${HOST}" ]     && JSON+=" \"{#HOST}\":\"${HOST}\","
    [ ! -z "${PORT}" ]     && JSON+=" \"{#PORT}\":\"${PORT}\","
    [ ! -z "${ITEMNAME}" ] && JSON+=" \"{#ITEMNAME}\":\"${ITEMNAME}\","

    # process extra fields
    for ((i = 1; i < ${#FIELDS[@]}; i++)); do
      # split field for key / value
      IFS='=' read -r KEY VALUE <<< "${FIELDS[i]}"
      KEY=$(echo "$KEY" | tr '[:lower:]' '[:upper:]')

      # save to a local var the KEY
      declare "LOCAL_${KEY}=${VALUE}"
    done

    # define an array of valid extra fields
    EXTRA_FIELDS=(
      URL_SSL_ALLOW_EXPIRED
      URL_SSL_ALLOW_INVALID
    )

    # add extra fields to json results set with default values if not overrided by CSV
    for VAR in "${EXTRA_FIELDS[@]}"; do
      LOCAL_VAR="LOCAL_${VAR}"
      if [ ! -z "${!LOCAL_VAR}" ]; then
        JSON+=" \"{#${VAR}}\":\"${!LOCAL_VAR}\","
        unset "$LOCAL_VAR"
# enable to always print default ssl options
#       else
#         JSON+=" \"{#${VAR}}\":\"${!VAR}\","
      fi
    done

    # print the composed json
    echo -e -n "    {${JSON%,} }, \n"
  done
}
...
\"data\":
  ["
  printCSVJSON "$1" | sed 'H;1h;$!d;x;s/\(.*\),/\1/'
  echo " ]
}"
}
...
getHTTPData() {
  ...
  # xtract the curl version transform to integer
  curlVersion="$(curl -V | sed -n 's/^curl \([0-9.]*\).*/\1/p')"
  curlVersionNum=$(echo "$curlVersion" | awk -F. '{print $1 * 10000 + $2 * 100 + $3}')

  # transform min version to integer
  curlVersionMin="7.75.0"
  curlVersionMinNum=$(echo "$curlVersionMin" | awk -F. '{print $1 * 10000 + $2 * 100 + $3}')

  if [ "$curlVersionNum" -ge "$curlVersionMinNum" ]; then
    # %{errormsg} is supported since curl 7.75.0 version
    curlResponse="$(curl -L -s -w "%{http_code};%{time_total};%{errormsg}" -H "User-Agent: $curlUserAgent" $curlArgs "$URL" -o /dev/null)"
    else
    # disable uncompatible options
  ...
  IFS=';' read -r -a curlData <<< "$(echo "$curlResponse" | sed 's/,/./g')"
  ...
  [ ! -z "${curlData[2]}" ] && errormsg="${curlData[2]}"
}
...
url.monitor() {
  ...
  #[ -z "$ssl_time_expire" ] && JSON+="\"ssl_time_expire\": 0, "
  #[ -z "$ssl_time_left" ] && JSON+="\"ssl_time_left\": 0, "

  # compose the data var
  #JSON+="\"http_schema\":\"${SCHEMA}\", "
  [ ! -z "$http_code" ]       && JSON+=" \"http_code\": $http_code,"
  [ ! -z "$time_total" ]      && JSON+=" \"time_total\": $time_total,"
  [ ! -z "$errormsg" ]        && JSON+=" \"errormsg\": \"$errormsg\"," || JSON+=" \"errormsg\": \"Ok\","
  [ ! -z "$ssl_time_expire" ] && JSON+=" \"ssl_time_expire\": $ssl_time_expire,"
  [ ! -z "$ssl_time_left" ]   && JSON+=" \"ssl_time_left\": $ssl_time_left,"
